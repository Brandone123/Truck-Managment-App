  ...
  <v-dialog v-model="dialog" max-width="600px">
    <v-card>
      <v-toolbar color="primary" dark density="compact">
        <v-toolbar-title>Copy Quiz Form</v-toolbar-title>
        <v-spacer></v-spacer>
...
const props = defineProps({
  ...
  form: {
    type: Object as PropType<OrientationQuiz>,
    required: true
  }
});
...
const quizFormCopy = ref<HTMLFormElement | null>(null)
...
const save = async () => {
  const formStatus = await quizFormCopy.value?.validate()
  if (!formStatus.valid) {
    return
  }
