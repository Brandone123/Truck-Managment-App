...
const records = ref<OrientationQuiz[]>([]);
...
const QuizFormTemplateAbortController = ref<AbortController | null>(null)


const freshQuizFormTemplateAbortController = () => {
    // If there's already an abortController, abort it
    if (QuizFormTemplateAbortController.value) {
        QuizFormTemplateAbortController.value.abort()
    }
    // Create a new AbortController
    QuizFormTemplateAbortController.value = new AbortController()
    return QuizFormTemplateAbortController.value
}
...
const loadRecords = async (): Promise<void> => {
    const controller = freshQuizFormTemplateAbortController()

    loading.value = true;

    const { data, error } = await useApiFetch('/api/recruiting/onboarding-orientation-management/orientation-quizzes', {
    ...
    if (error.value) {
        ...
        if (pagination.value.page === 1) {
            records.value = response.data as OrientationQuiz[];
