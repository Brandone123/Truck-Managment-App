<template>
          ...
          <v-row>
            <v-col cols="12" class="py-0">
              <v-textarea v-model="editedItem.description" hide-details variant="outlined" density="compact"
                label="Description" :rows="2"></v-textarea>
            </v-col>
            <v-col cols="12" class="py-0 mt-5 mb-2">
              <v-text-field v-model="editedItem.textValue" label="Value" placeholder="Type here..."
                hide-details variant="outlined" density="compact" :rules="[validation.required]"></v-text-field>
            </v-col>
            </v-row>
        </v-card-text>
      </v-expand-transition>
  ...
  const props = defineProps({
    modelValue: {
      type: Object as PropType<ShortAnswer>,
      required: true,
      default: {} as ShortAnswer,
    },
    expanded: {
      type: Boolean,
      default: true,
  ...
  const editedItem = ref<ShortAnswer>(props.modelValue);
