<template>
    <v-autocomplete v-model="selectedItem" :items="employees" :loading="loading" :hide-no-data="!loading" :flat="flat"
...
type EmployeeType = 'isAdmin' | 'isTechnician' | 'isDriver' | 'isOperator';

type EmployeeFilter = Partial<Record<EmployeeType, boolean>>;
...
const props = defineProps({
    ...
    filters: {
        type: Object as PropType<EmployeeFilter>,
        required: false,
        default: {}
    }
});
...
const { employees: employeeList, loadingEmployees: loading } = storeToRefs(commonListStore);
    ...
    if (Object.keys(props.filters)?.length) {
        return employeeList.value.filter(employee => {
            return Object.entries(props.filters).every(([key, value]) => {
                // Check if the employee's key matches the value
                return employee[key as keyof typeof employee] === value;
            });
        });
    }
...
const selectedItem = computed({
    ...
    set(value: any) {
        let selectedItem = employees.value.find(item => item.id == value)
