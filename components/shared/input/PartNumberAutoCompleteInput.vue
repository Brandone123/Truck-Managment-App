<template>
    <v-autocomplete v-model="selectedItem" :items="itemList" :no-data-text="noDataText" :loading="loading"
        ...
        :rules="rules" :item-title="itemTitleKey" :item-value="itemValueKey" @update:search="onSearch"
        @scroll-bottom="onScrollBottom" @update:focused="inputFocused = $event" no-filter>
...
type DefaultCategory = 'battery' | 'belt' | 'brakes' | 'cores' | 'electrical' | 'filters' | 'fluids' | 'oil' | 'parts' | 'suspension' | 'tires';
...
const props = defineProps({
    ...
    category: {
        type: String as PropType<DefaultCategory>,
        required: false,
    }
});
...
const itemTitleKey = "part_number"
...
const selectedItem = computed({
    ...
    set(value: any) {
        ...
        if (selection) {
            ...
            //selection[itemTitleKey]; // Update the search input with the selected item's name
        }
...
const filters = computed(() => {
    ...
    switch (props.category) {
        ...
        case 'oil':
            ...
            break;
        case 'parts':
            filters.category_id = 9;
            break;
        case 'suspension':
            filters.category_id = 10;
            break;
        case 'tires':
...
watch(search, (newVal, oldVal) => {
    ...
    if (currentSelection.value) {
        // console.log(currentSelection.value[itemTitleKey])
    }
    nextTick(() => {
...
const onSearch = (newSearch: string) => {
    ...
    // nextTick(() => {
    // if (selectedItem.value && (selectedItem.value as LineItem)?.id == props.modelValue) {
    //     return
    // }
    // Prevent triggering search when selecting an item from the list
    // if (items.value?.some((item) => item.id == (selectedItem.value as LineItem)?.id)) {
    //     return; // Do nothing if the input matches a selected item
    // }
