            ...
            <v-window-item v-for="tab in tabs" :value="tab.value">
                <v-card-text>
                    <v-data-table color="primary" :item-selectable="isItemSelectable" v-model="selectedIssues"
                        ...
                        :items="getFilteredIssueWorkOrders(tab.value)" :loading="loading">
...
const getIssueWorkOrder = computed(() => {
...
const getFilteredIssueWorkOrders = (tab: string) => {
    return getIssueWorkOrder.value.filter((issue) => issue.status === tab && issue.asset_name === getVehicleName(props.vehicleId));
};
