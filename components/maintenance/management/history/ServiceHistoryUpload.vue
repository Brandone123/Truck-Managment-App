  ...
  <v-dialog :modelValue="modelValue" @update:modelValue="updateModelValue" scrollable max-width="600px">
    <v-card>
      <v-toolbar color="primary" dark density="compact">
        <v-toolbar-title>Import Service History</v-toolbar-title>
        <v-spacer></v-spacer>
...
const uploadFile = async () => {
  if (files.value.length > 0) {
    ...
    try {
      ...
      await serviceHistoryStore.importServiceHistory(formData);
      layoutStore.setStatusMessage('Service history imported successfully', 'primary');
      emit('imported');
    } catch (error) {
      layoutStore.setStatusMessage('Failed to import service history', 'error');
