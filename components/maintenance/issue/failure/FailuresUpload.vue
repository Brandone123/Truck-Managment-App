  ...
  <v-dialog :modelValue="modelValue" @update:modelValue="updateModelValue" scrollable max-width="600px">
    <v-card>
      <v-toolbar color="primary" dark density="compact">
        <v-toolbar-title>Import Failures</v-toolbar-title>
        <v-spacer></v-spacer>
...
const uploadFile = async () => {
  if (files.value.length > 0) {
    ...
    try {
      ...
      await failureStore.importFailures(formData);
      layoutStore.setStatusMessage('Failures imported successfully', 'primary');
      emit('imported');
    } catch (error) {
      layoutStore.setStatusMessage('Failed to import failures', 'error');
