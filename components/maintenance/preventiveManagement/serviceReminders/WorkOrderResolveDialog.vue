<template>
  <div>
    <WorkOrderEditDialog v-model="workOrderDialog" :workOrder="workOrder" @save="saveworkOrder">
    ...
    <v-dialog v-model="dialog" max-width="800px" scrollable>
      <v-card>
        <v-toolbar color="primary" dark density="compact">
          <v-toolbar-title>Add To Work Order</v-toolbar-title>
          <v-spacer></v-spacer>
                ...
                <v-row class="my-5">
                  ...
                  <v-col cols="8">
                    <v-radio-group v-model="serviceReminderType">
                      <v-radio color="secondary" label="Add to New Work Order" value="new"></v-radio>
                      <v-radio color="secondary" label="Add to Existing Work Order" value="existing"></v-radio>
                    </v-radio-group>
                    <SharedInputWorkOrderAutoCompleteInput v-if="serviceReminderType == 'existing'" variant="outlined"
                      density="comfortable" :vehicleId="props.serviceReminder.vehicle_id" label="Please Select"
                      v-model="localReminderSchedule.work_order_id" :rules="[validation.required]" />
                  </v-col>
                </v-row>
                <!-- <v-row>
...
const workOrderDialog = ref<boolean>(false);
const workOrder = ref<Partial<WorkOrder>>({ vehicle_id: props.serviceReminder.vehicle_id })
...
const workOrderItem = ref({
  newworkOrderCheckbox: false,
  existingworkOrderCheckbox: false,
...
watch(
  () => dialog.value, (newVal) => {
    if (newVal) {
      ...
      workOrder.value.vehicle_id = props.serviceReminder?.vehicle_id
    } else {
...
const saveReminder = async () => {
  ...
  if (serviceReminderType.value == 'new') {
    localReminderSchedule.value.work_order_id = undefined
