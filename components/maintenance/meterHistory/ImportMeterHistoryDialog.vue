  ...
  <v-dialog :modelValue="modelValue" @update:modelValue="updateModelValue" scrollable max-width="600px">
    <v-card>
      <v-toolbar color="primary" dark density="compact">
        <v-toolbar-title>Import Meter History</v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-card-text>
        ...
        <v-file-input v-model="files" label="Select CSV File" prepend-icon="mdi-cloud-upload-outline"
          accept=".csv" variant="outlined"></v-file-input>
      </v-card-text>
      <v-divider></v-divider>
      <v-card-actions>
...
const uploadFile = async () => {
  if (files.value.length > 0) {
    ...
    formData.append('importType', selectedType.value) // Select the first file

    try {
      layoutStore.setOverlay(true);
      await meterHistoryStore.importMeterHistory(formData);
      layoutStore.setStatusMessage('Meter history imported successfully', 'primary');
      emit('imported');
    } catch (error) {
      layoutStore.setStatusMessage('Failed to import meter history', 'error');
