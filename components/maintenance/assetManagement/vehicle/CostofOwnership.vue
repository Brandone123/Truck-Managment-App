    ...
    <v-row>
        <v-col :cols="showSidePanel ? 12 : 6">
            <v-card>
                <div class="d-flex">
                    ...
                    <span class="ml-auto pa-2">
                        <v-select v-model="selectedYear" variant="outlined" flat density="compact" :items="yearOptions"
                ...
                <div>
                    ...
                    <client-only>
                        <apexchart type="bar" height="250" :options="generalLedgerOptions"
                            :series="generalLedgerOptions.series" />
                    </client-only>
                </div>
                ...
                <v-row>
                    <v-col v-for="(series, index) in generalLedgerOptions.series" :key="index" cols="4">
                        <div class="text-center">
                            <div>
                                <span
                                    :style="{ backgroundColor: series.color, borderRadius: '50%', display: 'inline-block', width: '12px', height: '12px', marginRight: '5px' }"></span>
                                {{ series.name }}
                            </div>
            ...
            <v-card height="400" class="mt-3">
                <div class="d-flex">
                    <div><v-card-title class="text-h7 font-weight-bold text-primary">Last Known Location</v-card-title>
                    </div>
                </div>
            ...
            <v-card height="170" class="mt-3">
                <div class="d-flex">
                    <div><v-card-title class="text-h7 font-weight-bold text-primary">Fuel Efficiency</v-card-title>
                    </div>
                </div>
                <div class="pa-5" style="font-size:small; color:grey; text-align: center;">
        ...
        <v-col :cols="showSidePanel ? 12 : 6">
            <AssetIssueList :vehicleName="selectedAsset.name" :vehicleId="selectedAsset?.id" />
            <v-card height="170" class="mt-3">
                <div class="d-flex">
                    <div><v-card-title class="text-h7 font-weight-bold text-primary">Renewal Reminders</v-card-title>
                    </div>
                    <span class="text-primary ml-auto mr-2 mt-3" style="font-size: 14px"><v-icon>mdi-plus</v-icon>Add
                        Renewal <span class="ml-2"> View All</span></span>
                </div>
                <div class="pa-5" style="font-size:small; color:grey; text-align: center;">
            ...
            <AssetIncompleteWorkOrderList />

            <AssetInspectionList />

            <AssetFaultList />

            <AssetRecalls class="mt-3" />
        </v-col>
    </v-row>
</template>

<script setup lang="ts">
import { ref, computed, onMounted, defineProps, type PropType } from 'vue';
import 'apexcharts/dist/apexcharts.css';
import googleMaps from '@/composables/googleMaps'
import AssetIssueList from './AssetIssueList.vue';
import AssetIncompleteWorkOrderList from './AssetIncompleteWorkOrderList.vue';
import AssetFaultList from './AssetFaultList.vue';
import AssetInspectionList from './AssetInspectionList.vue';
import AssetRecalls from './AssetRecalls.vue';
...
const props = defineProps({
    ...
    asset: {
        ...
        required: true,
        default: Object as Partial<Asset>
    }
});

const assetStore = useAssetStore();
const { selectedAsset, loadingComments } = storeToRefs(assetStore);
