          ...
          <v-row class="mt-1">
                    ...
                    <v-card-text>
                      <v-row>
                        <v-col cols="12" align="center">
                          <v-avatar size="150px">
                            <v-img :src="localTechnician?.profile_photo?.src || '/images/profile.png'"
                              alt="profile photo">
                          ...
                          <br/>
                          <input type="file" ref="profilePictureInput" accept="image/jpg, image/png"
                            @change="saveProfilePicture" style="display:none" />
                          <v-btn color="primary" @click="uploadProfilePicture">Update Profile Photo</v-btn>
                        </v-col>
                        <v-col cols="12" md="6">
                        ...
                        <!-- <v-col cols="12" md="6">
                          <v-text-field v-model="localTechnician.email" variant="outlined" flat label="Email"
                            density="compact" :rules="[validation.required]"></v-text-field>
                        </v-col> -->
                        <v-col cols="12" md="6">
                          <v-select v-model="localTechnician.department_id" variant="outlined" flat label="Department"
                            :items='formattedDepartmentList' item-title="full_name" item-value="id"
                            density="compact"></v-select>
                        </v-col>
                        <v-col cols="12" md="6">
                          <v-select v-model="localTechnician.role_id" variant="outlined" flat label="Role"
                            :items='roleList' item-title="name" item-value="id" density="compact"></v-select>
                        </v-col>
                        <v-col cols="12" md="6">
                          <v-select :items="['Salary', 'Hourly']" v-model="localTechnician.employment_type"
                  ...
                  <v-card class="mt-2">
                    ...
                    <v-card-text>
                      <v-row>
                        ...
                        <v-col cols="12" md="6">
                          <v-text-field label="License State" v-model="localTechnician.license_state" density="compact"
                            variant="outlined"></v-text-field>
                        </v-col>
                      </v-row>
                    </v-card-text>
                      ...
                      <v-row>
                        ...
                        <v-col cols="12" md="6">
                          <v-select label="Experience Level*" :rules="[validation.required]" :items="expertiseLevels"
                            item-value="value" item-title="text" v-model="localTechnician.experience_level"
                            density="compact" variant="outlined"></v-select>
                        </v-col>
                        <v-col cols="12" md="6">
                        ...
                        <v-col cols="12" md="6">
                          <v-text-field label="Hire Date*" v-model="localTechnician.hire_date"
                            :rules="[validation.required]" density="compact" variant="outlined"
                            type="date"></v-text-field>
                        </v-col>
                        <v-col cols="12" md="6">
                          <v-text-field label="Termination Date" v-model="localTechnician.termination_date"
                    ...
                    <v-card-text>
                      <v-row>
                        <v-col cols="12" class="d-flex justify-center">
                          <v-row>
                            <v-col cols="3">
                              <v-switch v-model="localTechnician.isAdmin" color="primary" label="Is Admin"
                                hide-details></v-switch>
                            </v-col>
                            <v-col cols="9">
                              <v-autocomplete :disabled="!localTechnician.isAdmin" variant="outlined" density="compact"
                                label="Admin Role">
                              </v-autocomplete>
                            </v-col>
                          </v-row>
                        ...
                        <v-col cols="12">
                          <v-row>
                            <v-col cols="3">
                              <v-switch v-model="localTechnician.isOperator" color="primary" label="Is Operator"
                                hide-details></v-switch>
                            </v-col>
                            <v-col cols="9">
                              <v-autocomplete :disabled="!localTechnician.isOperator" variant="outlined" density="compact"
                                label="Operator Role">
                              </v-autocomplete>
                            </v-col>
                          </v-row>
                        ...
                        <v-col cols="12">
                          <v-row>
                            <v-col cols="3">
                              <v-switch v-model="localTechnician.isTechnician" color="primary" label="Is Technician"
                                hide-details></v-switch>
                            </v-col>
                            <v-col cols="9">
                              <v-autocomplete :disabled="!localTechnician.isTechnician" variant="outlined" density="compact"
                        ...
                        <v-col cols="12">
                          <v-row>
                            <v-col cols="3">
                              <v-switch v-model="localTechnician.isDriver" color="primary" label="Is Driver"
                                hide-details></v-switch>
                            </v-col>
                            <v-col cols="9">
                              <v-autocomplete :disabled="!localTechnician.isDriver" variant="outlined" density="compact"
                                label="Driver Role">
                              </v-autocomplete>
                            </v-col>
                          </v-row>
            ...
            <v-col cols="12" md="6">
              <v-card class="mt-3">
                ...
                <v-card-text>
                  <v-row>
                    <v-col cols="12" v-for="(location, index) in localTechnician.locations" :key="index" class="pa-3">
                      <v-card-text>
...
//   const employeeId = localTechnician.value.id;
...
const addLocation = () => {
  localTechnician.value.locations?.push({
    id: null,
    employee_id: localTechnician.value.id as number,
    location_id: null,
    auto_clock_in: false,
...
const removeLocation = (index: number) => {
  localTechnician.value.locations?.splice(index, 1);
};

const localTechnician = ref<EmployeeInfo>(JSON.parse(JSON.stringify(props.technician)));

const title = computed(() => (props.technician && props.technician.id ? 'Edit Technician Profile' : 'Create New Technican Profile'));
...
watch(
  () => props.technician,
  (newEmployee) => {
    localTechnician.value = JSON.parse(JSON.stringify(newEmployee)) as EmployeeInfo
    // localTechnician.value.email = employeeWithEmail.value
  })
...
const saveProfilePicture = async (e: Event) => {
  const files = (e.target as HTMLInputElement).files as FileList
  const fileArray = Array.from(files);

  if (fileArray.length > 0) {
    const { data, error } = await mediaStore.uploadFile(fileArray[0]);
    if (data) {
      localTechnician.value.profile_photo = data;
    }
  }
