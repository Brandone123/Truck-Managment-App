...
const availableDays = computed(() => {
    if (employeeLeavePolicy.value) {
        return ((employeeLeavePolicy.value?.assigned as number) - (employeeLeavePolicy.value?.used as number))
    }
    return 0
})
...
<template>
    <v-dialog v-model="dialog" :width="employeeProfileErrors.length > 0 ? '600px' : '800px'">
        ...
        <v-card v-if="employeeProfileErrors.length">
            <v-card class="grey-background">
                ...
                <v-card-text>
                    <ul>
                        <li v-for="(message, i) in employeeProfileErrors" :key="i">{{ message }}</li>
                    </ul>
                </v-card-text>
                <v-divider></v-divider>
                <v-card-actions>
                            ...
                            <v-card flat>
                                ...
                                <v-card-text>
                                    ...
                                    <p>
                                        ...
                                        <span>
                                            {{ employeeLeavePolicy.policy_activated ? 'Active' : 'Inactive' }}
                                        </span>
                                    </p>
                                    <p>
                                    ...
                                    <p><b>Available Days: </b>
                                        <span>
                                            {{ (employeeLeavePolicy.assigned as number) - (employeeLeavePolicy.used as
        number)
                                            }}
                                        </span>
                                    </p>
                                    <p><b>Leave Request Minimum Notice Period (Days): </b>
                                        <span>
                                            {{ employeeLeavePolicy.policy?.min_notice_period }} days from Current Day
                                        </span>
                                    </p>
                                    <p></p>
                                </v-card-text>
                        ...
                        <v-row v-if="availableDays > 0">
                            ...
                            <v-col cols="12" md="6" class="pb-0">
                                <v-text-field type="date" variant="solo" flat density="compact" label="End Date"
                                    ...
                                    :max="getEndDateMax" :rules="rules.end_date"></v-text-field>
                            </v-col>
                            <v-col cols="12" md="6" class="pb-0">
