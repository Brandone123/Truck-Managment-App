...
export default {
  ...
  setup(props) {
    ...
    const leaveApprover = computed(() => {
      return employeeList.value.find(
        (item) => item.user_id === employee.value.approver_id
      );
    });
    ...
    const totalRequest = computed(() => {
      return timeoffRequestList.value.filter(
        (item: TimeoffRequest) => item.user_id == employee.value.user_id
      ).length;
    });

    const availableDays = computed(() => {
      return props.timeoffInfo.filter((item) => typeof item === "number")
        .length;
    });
    ...
    const chartOptions = computed(() => {
      ...
      const employeeTimeOff = timeoffRequestList.value.filter(
        (item) => item.user_id === employee.value.user_id
      );
      const leaveType = employeeTimeOff.find((item) =>
        leaveTypeList.value.some(
          (leaveType) => leaveType.id === item.attendance_leave_type_id
        )
      );
      const leaveTypeColor = leaveTypeList.value.find(
        (item) => item.id === leaveType?.attendance_leave_type_id
      )?.color;
          ...
          <v-card-text style="font-size: 17px">
            <div style="display: flex" class="mb-5">
              ...
              <div style="flex: 1" class="mr-2">
                <div class="mb-4">
                  <span class="font-bold">Available Days: </span>
                  {{ availableDays }} days out of {{ daysInMonthCount }}
                </div>
                <span class="font-bold">Approver: </span
                >{{
                  leaveApprover
                    ? leaveApprover.full_name
